...
const model = genAI.getGenerativeModel({
  // Changed to the model you specified.
  ...
  try {
    ...
    const prompt = `
      ...
      **Process:** You MUST follow these steps precisely. At each step requiring information, you are mandated to use your search tool to gather live, real-time data.
      ...
      3.  **Structure the Curriculum & Find Resources:** Organize the synthesized competencies into 7-10 progressive, logical learning modules, beginning the curriculum with a brief excitedly toned intro into the history of the occupation of "${jobTitle}" and what foundational knowledge should be thought of as mandatory to master when seeking work as a "${jobTitle}". For EACH key competency or skill within a module, perform a new, targeted search to find specific, high-quality, FREE online learning resources (e.g., a class, a bootcamp, a tutorial, an article, a video series, hands-on lab, certificate course, online lectures, official documentation, etc.) or almost free virtual resources that might be of use (such as free trial offerings, discounts, specials for students, or super low cost programs). Make sure there is a working link and short descriptive blurb for every resource that is outputted. 
      ...
      **Output Instructions:** The final output MUST be a single, clean, uniformally formatted, properly outlined Markdown document. Do NOT include any conversational text, apologies, or explanations of your process in the final output. For every resource, provide a direct, clickable Markdown link.
    `;
